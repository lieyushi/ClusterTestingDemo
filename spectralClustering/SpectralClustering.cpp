...
void SpectralClustering::readCSV(const char* csvName)
{
	std::cout << "Started reading file..." << std::endl;

	ifstream fin(csvName, ios::in);
	if(!fin)
	{
...
void SpectralClustering::printGroup()
{
	assert(!group.empty());

	ofstream fout("group.txt", ios::out);
	if(!fout)
	{
...
void SpectralClustering::readTXT(const char* txtName)
{
	std::ifstream fin(txtName, ios::in);
	if(!fin)
	{
...
void SpectralClustering::readPoint(const char* txtName)
{
...
#pragma omp parallel for schedule(dynamic) num_threads(8)
	for (int i = 0; i < numOfNodes; ++i)
	{
		for (int j = 0; j < numOfNodes; ++j)
		{
			distanceMatrix(i,j) = (coordinates[i]-coordinates[j]).norm();
		}
	}
}
...
/* void print vtk file */
void SpectralClustering::printVTK()
{
	ofstream fout("clustering.vtk", ios::out);
	if (!fout)
	{
